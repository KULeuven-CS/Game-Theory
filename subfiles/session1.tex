...
\begin{solution}
	...
	Let $s_i(a_j)$ be the probability that player $i$ plays action $a_j$ in his mixed strategy.
	The expected utility of $\overline{s}_i$ is:
	\begin{align}
		u_i(\overline{s}_i, s_{\--i}) &= \sum\limits_{j=1}^{|A_i|} u_i(a_j, s_{\--i}) s_i(a_j) & \forall s_{\--i} \in S_{\--i}\\
		s_i(a_j) &\geq 0 & 1 \leq j \leq |A_i| \\
