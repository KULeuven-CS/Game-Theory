...
\begin{solution}
	...
	Probably they made an error in their question and it should read ``two VERY weakly dominant pure strategies''. Because equality is sufficient for this property these strategies CAN coexist.
...
Every strategy that is part of Nash Equilibrium is by definition dominant otherwise the player would pick an other, better, strategy for himself and thus deviate from the equilibrium.
...
\begin{solution}
	...
	\end{equation}
	This unequality clearly can not hold under the given assumptions. Indeed if $s'_i = \overline{s}_i$ \autoref{eq:uneq2} can no longer hold and still holds $u_1(s'_i,s_{\--i}) \geq u_i(\overline{s}_i,s_{\--i})$. By condradiction we proved the given statement. $\blacksquare$ 
\end{solution}
