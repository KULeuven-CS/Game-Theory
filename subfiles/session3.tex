...
%TODO: Maybe not needed to explicitly list 4 P2 strategies. Cartesian product of decision nodes. But there is only ONE decision node (right)!! So cartesian product results in only two options.
\begin{center}
...
%TODO: verify
	%Subgame perfect equi should also be a Nash equi!!!
	\item Next we have to look at Subgame Perfect NEs (SPNEs).
		...
		If we consider the second node of player one as subgame we see that $Steal$ is the best option here so there is no profitable deviation for this subgame ($10>5$).
		...
		If player two choses $Trust$ (instead of restriction $Distrust$) it will result in a zero payoff ($0<2$).
		And we already considered the options of player one in the smaller subgame.
		The last subgame is the whole game and we already know the outcome here.
		Because we found no profitable deviation, this profile is a \textbf{Subgame Perfect Nash Equilibrium}.
		\textcolor{blue}{$\Big((Not Play, Share),Distrust)\Big)$}:\\
		If we consider the second node of player one as subgame we see that $Steal$ is the best option here for player one ($10>5$).
		This means there IS a profitable deviation in some subgame so this strategy profile can NOT be a Subgame Perfect Nash Equilibrium. 
